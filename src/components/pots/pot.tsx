...
export function Pot({ pot }: PotProps) {
  return (
    <div
      ...
      style={{ "--pot-color": pot.theme } as CSSProperties}
    >
      ...
      <div className={styles["pot-details"]}>
        <div className={classNames(styles["pot-total-title"], "text-preset-4")}>
          Total saved
        </div>
        <div className={classNames(styles["pot-total"], "text-preset-1")}>
          ${pot.total.toFixed(2)}
        </div>
        {/* <progress className={styles["pot-progress"]} max={pot.target} value={pot.total} /> */}
        <Progress className={styles["pot-progress"]} color={pot.theme} max={pot.target} value={pot.total} />
        <div
          className={classNames(styles["pot-percentage"], "text-preset-5-bold")}
        >
          {toPercentage(pot.total / pot.target)}%
        </div>
        <div
          className={classNames(styles["pot-target-title"], "text-preset-5")}
        >
